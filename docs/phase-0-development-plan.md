...
> 状态：已完成；P0-00 ～ P0-10 均已通过，Phase 1 入口已开放
> 计划版本：1.1  
> 制定日期：2026-07-23  
...
- [x] 已建立 `fishtongue-desktop` Git 仓库，并保留 Lexurgy App 的 GPL-3.0 许可证与原作者声明；
- [x] 已记录所采用的 Lexurgy App 上游提交哈希，且仓库配置了 `upstream` 远程地址；
- [x] `npm ci` 可以从锁文件完成全新安装；
- [x] 原项目单元测试与 Cypress 组件测试的基线结果已记录；
- [x] Next.js 使用 `output: "export"`，`npm run build` 生成 `out/` 静态文件；
- [x] 运行时不使用 SSR、API Routes、Server Actions、NextAuth、Neo4j 或 Node.js 服务端；
- [x] Tauri 2 开发模式可以启动 FishTongue 独立窗口；
- [x] CodeMirror 可以输入、删除、选择、撤销、重做和自动补全括号；
- [x] Lexurgy 规则关键字能显示语法高亮，行号正常；
- [x] 桌面音变页面只通过 `SoundChangeEngine` Port 表达引擎需求，不直接依赖 Tauri 或远程 API；
- [x] 未接入的音变运行和规则校验不会请求公网，也不会展示伪造结果；
- [x] `npm run tauri:build` 能生成 Windows NSIS 安装程序；
- [x] 在一台独立 Windows 环境中完成安装、卸载和重复安装验收；维护者确认哈希校验通过并接受验收结果；
- [x] 已提交 Phase 0 验收报告和测试记录；外部电脑验收无截图，已在报告中如实注明；
- [x] 没有 P0/P1 级缺陷，且所有已知限制均已写入验收报告。
