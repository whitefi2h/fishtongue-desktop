...
状态：**外部核心功能验收通过；正常退出被误判为异常退出的问题正在修复复验，Phase 2 尚未结项。**
...
- 安装包大小：`50,934,103` 字节
- 安装包 SHA-256：
  `1DDED1B0756749CA017A8A7AD2C668CF64DED83CC477DA259192698EA832CA2F`
- 签名状态：未签名（开发阶段安装包）
...
2026-07-26 正常退出修正版完整运行结果：**通过**。Jest 16 个测试套件、64 项测试通过；
Cypress 15 个规格、83 项测试通过；Rust 16 项测试通过；Lexurgy 全部上游和桌面
