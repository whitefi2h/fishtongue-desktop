...
import { PhonologyRepository } from "@/fishtongue/application/ports/Phase6Ports";
import { ResolvedStageState, StageComponentOverride } from "@/fishtongue/domain/models";
...
export default class StageStateResolver {
  ...
  private async resolveInternal(stageId: string, ancestors: string[]): Promise<ResolvedStageState> {
    if (ancestors.includes(stageId)) {
      throw new Error("阶段的数据继承形成了循环，请先修正数据基础阶段。");
    }
    ...
    const resolved: ResolvedStageState = {
      stage,
      lineage: stage.kind === "internal_default" ? [stage.id] : [...base.lineage, stage.id],
  ...
  private async loadLanguageState(languageId: string): Promise<ResolvedStageState> {
    ...
    const [lexemes, morphemes, evolution, inflection, profiles, phonology] = await Promise.all([
      ...
      this.phonology?.getOrCreate(languageId),
    ]);
    return {
      stage,
      lineage: [stage.id],
      ...
      components: {
        ...
        phonology: phonology as unknown as Record<string, unknown> | undefined,
      },
    };
  }
}
...
function emptyComponents(): ResolvedStageState["components"] {
  return { lexicon: {}, morphemes: {}, wordgen: {} };
}

function cloneComponents(value: ResolvedStageState["components"]): ResolvedStageState["components"] {
  return JSON.parse(JSON.stringify(value)) as ResolvedStageState["components"];
}

function applyOverride(state: ResolvedStageState, override: StageComponentOverride): void {
  if (["evolution", "inflection", "phonology"].includes(override.componentType)) {
    if (override.operation === "remove") {
      delete state.components[override.componentType as "evolution" | "inflection" | "phonology"];
    } else if (override.operation === "merge") {
      state.components[override.componentType as "evolution" | "inflection" | "phonology"] = {
        ...(state.components[override.componentType as "evolution" | "inflection" | "phonology"] ?? {}),
        ...override.payload,
      };
    } else {
      state.components[override.componentType as "evolution" | "inflection" | "phonology"] = { ...override.payload };
    }
    return;
  }

  const componentType = override.componentType as "lexicon" | "morphemes" | "wordgen";
  const collection = state.components[componentType] ?? {};
  state.components[componentType] = collection;
  if (override.operation === "remove") {
    delete collection[override.targetId];
  } else if (override.operation === "merge") {
    collection[override.targetId] = { ...(collection[override.targetId] ?? {}), ...override.payload };
  } else {
