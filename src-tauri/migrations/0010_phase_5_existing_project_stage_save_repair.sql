...
-- Migration v9 introduced the atomic stage write command, but some projects
-- had already recorded schema version 9 before that repair was distributed.
-- A migration is only executed once, so those projects never received the
-- command table.  Re-introduce the objects in a new, additive migration.
CREATE TABLE IF NOT EXISTS language_stage_write_commands (
...
-- Recreate the trigger so projects that received an incomplete v9 repair and
-- projects created by the fixed build converge on the same implementation.
DROP TRIGGER IF EXISTS execute_language_stage_write;
