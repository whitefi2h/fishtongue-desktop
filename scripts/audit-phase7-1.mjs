...
for (const required of [
  ...
  if (!migration.includes(required))
    failures.push(`Schema v16 is missing ${required}`);
}
...
for (const required of [
  ...
  if (!rust.includes(required))
    failures.push(`Project history is missing ${required}`);

const migrations = read("src-tauri/src/migrations.rs");
if (
  !migrations.includes("DATABASE_SCHEMA_VERSION: u32 = 16") &&
  !migrations.includes("DATABASE_SCHEMA_VERSION: u32 = 17")
)
  failures.push("Current schema is older than the Phase 7.1 v16 baseline");

const session = read(
  "src/fishtongue/application/services/ProjectSessionService.ts"
);
for (const required of [
  "runProjectOperation",
  ...
  if (!session.includes(required))
    failures.push(`Project session is missing ${required}`);
}
...
for (const required of [
  ...
  "stageReadyLanguageId",
  "STAGE_LOAD_TIMEOUT_MS",
  "重新读取阶段",
  "setStageReadyLanguageId(created.id)",
]) {
  if (!desktop.includes(required))
    failures.push(`Desktop navigation/history is missing ${required}`);
}
...
for (const required of [
  ...
  if (!lexicon.includes(required))
    failures.push(`Stage lexicon is missing ${required}`);
}

const registry = read("src/fishtongue/ui/prototype/registry.ts");
if (registry.includes('id: "global-undo"'))
  failures.push("Obsolete planned global-undo entry remains visible");

const packageJson = JSON.parse(read("package.json"));
if (!/^0\.7\.0-phase\.7\.(1|2)$/.test(packageJson.version))
  failures.push("Package version no longer includes the Phase 7.1 baseline");
if (!packageJson.scripts["verify:phase7-1"])
  failures.push("verify:phase7-1 is missing");
...
console.log(
  "Phase 7.1 history, stage lexicon, navigation and documentation audit passed."
);
