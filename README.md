...
> Sidecar、音变预览、屈折预览和 Schema v2 已完成开发与自动验收。首轮外部验收
> 已通过安装包哈希与无 Java 环境检查；真实新项目问题已经修复并通过复验。第二轮
> 验收发现安装版 Java 无法识别 Tauri 的 verbatim 资源路径，Sidecar 路径修正版
> 正在进行针对性复验，Phase 2 尚未结项。其他设计原型入口不代表相应能力已经实现。
