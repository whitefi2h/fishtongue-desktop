...
> Sidecar、音变预览、屈折预览和 Schema v2 已完成开发与自动验收。外部验收已确认
> 安装包哈希、无 Java 环境、真实新项目和语言持久化；随后发现并修复了 Windows
> Sidecar 路径、音变预览响应缺少空字段时的界面白屏，以及正式词典没有新建入口
> 的问题。“当前词典”复测已通过；窗口关闭权限问题已完成修复和本机原生验证，
> 正在等待外部复验，Phase 2 尚未结项。
> 其他设计原型入口不代表相应能力已经实现。
