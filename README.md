...
Phase 7.2 正在实现统一演化工作台、不可变版本和运行、IPA 默认输入、目标拼写确认、四类落点的原子提交以及演化图。当前数据库开发版本为 Schema v17。开发计划和人工验收入口见 [`docs/phase-7-2/`](docs/phase-7-2/)。

本阶段尚未通过独立 Windows 人工验收，因此不能视为正式结项；Phase 7.1 仍是最近的已验收发布基线。
...
侧栏可切换已启用的服务商与模型，提案先进入对应正式编辑器或批量审核窗口，再由用户确认保存。
批量词条/语素提案最多 50 项，以双主题可编辑表格选择接受或拒绝，并在保存后立即刷新当前
工作区。造词、演化和屈折提案在进入编辑器或引擎前会执行格式转换与严格校验，模型返回的
别名、简写或无效 Lexurgy 规则不会直接进入正式数据链路；后续对话也会在受限预算内读取本
...
- 阶段时间顺序：使用 `language_stages.chronology_parent_id`；
- 阶段数据继承：使用 `language_stages.data_base_stage_id`；
